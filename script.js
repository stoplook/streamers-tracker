...
function detectLiveFromRss(rssText) {
  ...
  if (/liveBroadcastContent/i.test(rssText) && />\s*live\s*</i.test(rssText)) return true;
  try {
...
function detectLiveFromChannelHtml(html) {
  if (!html || typeof html !== "string") return false;
  if (/"isLive"\s*:\s*true/i.test(html)) return true;
  if (/"liveStreamability"\s*:\s*"LIVE"/i.test(html)) return true;
  if (/"status"\s*:\s*"LIVE"/i.test(html) && /videoId/i.test(html)) return true;
  return false;
}
...
async function getYoutubeStatusStrict(youtubeUrl) {
  ...
  try {
    channelId = await resolveYoutubeChannelId(youtubeUrl);
    if (channelId) {
      const rssUrl = ytRssUrl(channelId);
      const rssText = await fetchTextAnyWay(rssUrl, 16000);
      if (detectLiveFromRss(rssText)) {
        setTtl(statusCache, key, true);
        return true;
      }
    }
...
function createStreamerCard(s, data) {
  ...
  // сохраняем данные прямо в карточке (важно для часов)
  ...
  adminActions.innerHTML = `
    <button class="admin-mini" type="button">✏</button>
    <button class="admin-mini" type="button">🗑</button>
  `;
  ...
  btns[1].onclick = async () => {
    try {
      ...
      await adminApi(`/streamers?id=${encodeURIComponent(s.id)}`, {
        method: "DELETE"
      });
  ...
  if (s.twitch) iconGroup.appendChild(makeIconBtn(s.twitch, ICONS.twitch));
  if (s.youtube) iconGroup.appendChild(makeIconBtn(s.youtube, ICONS.youtube));
  if (s.kick) iconGroup.appendChild(makeIconBtn(s.kick, ICONS.kick));
  if (s.tiktok) iconGroup.appendChild(makeIconBtn(s.tiktok, ICONS.tiktok));
